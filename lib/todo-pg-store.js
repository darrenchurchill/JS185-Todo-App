...
class TodoPGStore {
  ...
  constructor(session) {
    ...
    // TODO: remove this assignment once the data is in Postgres
  ...
  sortedTodoLists() {
    ...
    return this.#todoListsDeepCopy().map((list) =>
      TodoPGStore.assignMetadata(list)
    );
  }
  ...
  static sortedTodoList(todoList) {
    if (TodoList.isTodoListLike(todoList)) {
      ...
      result.todos.sort(TodoPGStore.#compareTodos);
      return TodoPGStore.assignMetadata(result);
    }
