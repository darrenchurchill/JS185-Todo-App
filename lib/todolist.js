...
class TodoList {
  ...
  static isTodoListLike(value) {
    return (
      ...
      value.todos.every((todo) => Todo.isTodoLike(todo))
    );
  }

  /**
   * Return a JSON value representing this `TodoList`'s core data properties.
   * @returns {TodoListQueryResult} the JSON-ified object
   */
  toJSON() {
    const listID = this.getID();
    const listTitle = this.getTitle();
    return {
      id: listID,
      title: listTitle,
      length: this.size(),
      countDone: this.allDone().size(),
      done: this.isDone(),
      todos: this.toSortedArray().map((todo) => {
        return {
          ...todo.toJSON(),
          listID,
          listTitle,
